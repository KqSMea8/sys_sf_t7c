...
function commonAjaxSubmits(url,formObj){
    if(!formObj||formObj==''){
        var formObj="form";
    }
    if(!url||url==''){
...
function progressBar(evt) {
    ...
    var per = Math.floor(100 * loaded / tot); //已经上传的百分比
  	if($('#progress_percent').length > 0){
    	$('#progress_percent').html('<b>'+per.toFixed(1)+'%</b>');
    	$('.pbar').children('.ui-progressbar-value').css('width', per.toFixed(1)+'%');
    }
}