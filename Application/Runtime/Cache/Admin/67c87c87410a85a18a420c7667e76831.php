...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>文书列表-<?php echo ($site["SITE_INFO"]["name"]); ?></title>
        <?php $addCss=""; $addJs=""; $currentNav ='文书列表'; ?>
            ...
            <?php if(is_array($sub_menu)): foreach($sub_menu as $key=>$sv): if($sv["sub3"] != 'false'): ?><li class="togellernav"><a><?php echo ($sv["title"]); ?></a>
                ...
                <div id="Right">
                    <div class="Item hr">
                        ...
                        <div style="width: 100px;float: right;"><button type="button" class="btn" onclick="window.location.href='<?php echo U('Writ/writ_add',['type'=>$_GET['type']]);?>'">添加文书</button></div>
                    </div>
                    <form action="" method="get" id="quickForm">
                        关键词:
                        <input type="text" name="keyword" class="input uname" size="40" value="<?php echo ($_GET['keyword']); ?>" style="height: 30px;line-height: 30px" />
                    ...
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr align="center" id="<?php echo ($vo["id"]); ?>">
                                ...
                                <td>[ <a href="/index.php/conist/Writ/writ_add?id=<?php echo ($vo["id"]); ?>">编辑 </a> ] [ <a link="<?php echo U('Writ/writ_del/',array('id'=>$vo['id']));?>" href="javascript:void(0)" name="<?php echo ($vo["title"]); ?>" class="del">删除 </a> ] [ <a href="/index.php/conist/Writ/message_add?id=<?php echo ($vo["id"]); ?>&title=<?php echo ($vo["title"]); ?>">推送 </a> ]</td>
    ...
    $(function(){
        ...
        <script type="text/javascript">
            ...
            function changeAttr(id,v){
                $.get('<?php echo U("Writ/changeAttr");?>?id='+id,function(data){
                    if(data.status==1){
                        $(v).html(data.info);
                    }
                });
