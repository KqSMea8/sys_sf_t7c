...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>公证处列表-<?php echo ($site["SITE_INFO"]["name"]); ?></title>
        ...
        <div id="Right" style="float:none;width:100%">
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab">
                ...
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr align="center" id="<?php echo ($vo["id"]); ?>">
                        ...
                        <td><?php echo ($vo["jgslsj"]); ?></td>
                        <td><?php echo ($vo["fzr"]); ?></td>
                        <td><?php echo ($vo["qy"]); ?></td>
                        <td><?php echo (date("Y-m-d H:i:s",$vo["createtime"])); ?></td>
                        <td><a id="<?php echo ($vo["id"]); ?>" class="memberList_gzc" href="javascript:;">人员列表 </a></td>
                        <td>
<!--                             <?php if($cate == 1): ?>[ <a id="<?php echo ($vo["id"]); ?>" class="memberList_gzc" href="javascript:;">人员列表 </a> ]<?php endif; ?> -->
                            [<a id="<?php echo ($vo["id"]); ?>" class="editList_gzc" href="javascript:;">编辑 </a> ]
                            [<a link="<?php echo U('Institution/gzc_del/',array('id'=>$vo['id']));?>" href="javascript:void(0)" name="<?php echo ($vo["name"]); ?>" class="del">删除 </a> ]
                        </td>
...
<script type="text/javascript">
            ...
            $('body').on('click','.memberList_gzc',function(){
                var id=$(this).attr('id');
                var url='/index.php/conist/Institution/gz_member_add?gid='+id;
                $("#btnAdd",window.parent.document).attr('onclick','showFirme("'+ url +'")');
                $("#btnAdd",window.parent.document).html('添加公证员');
                $("#btnAdd",window.parent.document).parent().show();
                var urls='/index.php/conist/Institution/gz_member?id='+id;
                showFirme(urls);
            })
            $('body').on('click','.editList_gzc',function(){
                var id=$(this).attr('id');
                var url='/index.php/conist/Institution/gzc_add?id='+id;
                showFirme(url);
            })
