...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>普法宣传列表-<?php echo ($site["SITE_INFO"]["name"]); ?></title>
        <?php $addCss=""; $addJs=""; $currentNav ='普法宣传列表'; ?>
    ...
    <div class="subMenuList">
        ...
        <ul>
            <?php if(is_array($sub_menu)): foreach($sub_menu as $key=>$sv): if($sv["sub3"] != 'false'): ?><li class="togellernav"><a><?php echo ($sv["title"]); ?></a>
                ...
                <div id="Right">
                    <div class="Item hr">
                        <div class="current">普法<?php echo ($cate); ?>列表</div>
                        <div style="float: right;"><button type="button" class="btn" onclick="window.location.href='<?php echo U('Popularizelaw/popularizelaw_add');?>'">添加普法宣传文章</button><button type="button" class="btn" onclick="window.location.href='<?php echo U('Popularizelaw/video_add');?>'">添加普法宣传视频</button></div>
                    </div>
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr align="center" id="<?php echo ($vo["id"]); ?>">
                                ...
                                <td><a href="javascript:void(0);" title="激活" onclick="changeStatus(<?php echo ($vo["id"]); ?>,this)"><img src="/Public/Img/action_<?php echo ($vo["status"]); ?>.png" border="0"></a></td>
                                <td>

                                <?php if($vo['cate'] == 1): ?>[ <a href="/index.php/conist/Popularizelaw/popularizelaw_add?id=<?php echo ($vo["id"]); ?>">编辑 </a> ]
                                <?php elseif($vo['cate'] == 2): ?>
                                [ <a href="/index.php/conist/Popularizelaw/video_add?id=<?php echo ($vo["id"]); ?>">编辑 </a> ]
                                <?php else: endif; ?>
                                [ <a link="<?php echo U('Popularizelaw/popularizelaw_del/',array('id'=>$vo['id']));?>" href="javascript:void(0)" name="<?php echo ($vo["name"]); ?>" class="del">删除 </a> ]
                                </td>
    ...
    $(function(){
        ...
        <script type="text/javascript">
            ...
            function changeAttr(id,v){
                $.get('<?php echo U("Popularizelaw/changeAttr");?>?id='+id,function(data){
                    if(data.status==1){
                        $(v).html(data.info);
                    }
                });
            ...
            function changeStatus(id,v){
                $.get('<?php echo U("Popularizelaw/changeStatus");?>?id='+id,function(data){
                    if(data.status==1){
                        $(v).html(data.info);
                    }
                });
