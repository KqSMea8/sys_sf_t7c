...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<head>
		...
		<?php $addCss=""; $addJs=""; $currentNav ='管理员列表'; ?>
            ...
            <?php if(is_array($sub_menu)): foreach($sub_menu as $key=>$sv): if($sv["sub3"] != 'false'): ?><li class="togellernav"><a><?php echo ($sv["title"]); ?></a>
				...
				<div id="Right">
					<div class="Item hr">
						...
						<div style="width: 100px;float: right;"><button type="button" class="btn" onclick="window.location.href='<?php echo U("Access/add_admin");?>'">添加管理员</button></div>
					</div>
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab">
						...
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr align="center" aid="<?php echo ($vo["aid"]); ?>">
								...
								<td>
									<?php if($vo["aid"] == 1): ?><a class="stateClick" href="javascript:void(0);" index="<?php echo ($vo["aid"]); ?>" title="" >--</a><?php else: ?>
										<?php if($vo["status"] == 1): ?><a class="stateClick " href="javascript:void(0); " index="<?php echo ($vo["aid"]); ?> "  title=" " onclick="onstate(<?php echo ($vo["aid"]); ?>,this) "><img src="/Public/Img/action_1.png " border="0 "></a>
									    <?php else: ?>
									    	<a class="stateClick " href="javascript:void(0); " index="<?php echo ($vo["aid"]); ?> "  title=" " onclick="onstate(<?php echo ($vo["aid"]); ?>,this) "><img src="/Public/Img/action_0.png " border="0 "></a><?php endif; endif; ?></td>
                                <td><?php if($vo["email"] == C( 'ADMIN_AUTH_KEY') ): ?>--<?php else: ?>[ <a href='/index.php/conist/Access/editAdmin?aid=<?php echo ($vo["aid"]); ?>'>编辑</a> ][ <a link="<?php echo U( 'Access/admin_del/',array( 'aid'=>$vo['aid']));?>" href="javascript:void(0)" name="<?php echo ($vo["name"]); ?>" class="del">删除 </a> ]<?php endif; ?>
								</td>
							</tr><?php endforeach; endif; else: echo "" ;endif; ?>
						<tr><td><td colspan="6" align="right"><?php echo ($page); ?></td></td></tr>
					</table>
				</div>
			</div>
		</div>
...
<script type="text/javascript">
	// $(".stateClick").click(onstate(<?php echo ($vo["aid"]); ?>,this));

	function onstate(id, v) {
		$.ajax({
			type: "get",
			url: "<?php echo U("Access/admin_change");?>",
			data: {
				aid: id
			},
			dataType: "json",
