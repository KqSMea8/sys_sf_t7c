...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>费用列表-<?php echo ($site["SITE_INFO"]["name"]); ?></title>
        <?php $addCss=""; $addJs=""; $currentNav ='费用列表'; ?>
    ...
    <div class="subMenuList">
        ...
        <ul>
            <?php if(is_array($sub_menu)): foreach($sub_menu as $key=>$sv): if($sv["sub3"] != 'false'): ?><li class="togellernav"><a><?php echo ($sv["title"]); ?></a>
                ...
                <div id="Right">
                    <div class="Item hr">
                        <div class="current">费用列表</div>
                        <div style="width: 100px;float: right;"><button type="button" class="btn" onclick="window.location.href='<?php echo U('Handle/price_add');?>'">添加费用</button></div>
                    </div>
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr align="center" id="<?php echo ($vo["id"]); ?>">
                                ...
                                <td>[ <a href="/index.php/conist/Handle/price_add?id=<?php echo ($vo["id"]); ?>">编辑 </a> ] [ <a link="<?php echo U('Handle/price_del/',array('id'=>$vo['id']));?>" href="javascript:void(0)" name="<?php echo ($vo["title"]); ?>" class="del">删除 </a> ]</td>
    ...
    $(function(){
        ...
        <script type="text/javascript">
            ...
            function changeAttr(id,v){
                $.get('<?php echo U("News/changeAttr");?>?id='+id+'&table=price',function(data){
                    if(data.status==1){
                        $(v).html(data.info);
                    }
                });
