...
<html xmlns="http://www.w3.org/1999/xhtml">
    <head>
        ...
        <title>考试类型列表-<?php echo ($site["SITE_INFO"]["name"]); ?></title>
        <?php $addCss=""; $addJs=""; $currentNav ='考试类型列表'; ?>
    ...
    <div class="subMenuList">
        ...
        <ul>
            <?php if(is_array($sub_menu)): foreach($sub_menu as $key=>$sv): if($sv["sub3"] != 'false'): ?><li class="togellernav"><a><?php echo ($sv["title"]); ?></a>
                ...
                <div id="Right">
                    <div class="Item hr">
                        <div class="current">考试类型列表</div>
                        <div style="width: 100px;float: right;"><button type="button" class="btn" onclick="window.location.href='<?php echo U('Popularizelaw/exam_type_add');?>'">添加考试类型</button></div>
                    </div>
                    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="tab">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr align="center" id="<?php echo ($vo["id"]); ?>">
                                ...
                                <td><?php echo ($vo["subject_num"]); ?></td>
                                <td><?php echo ($vo["total_points"]); ?></td>
                                <td>
                                <?php if($vo["state"] == 1): ?>平均分配
                                <?php else: ?>
                                指定分配<?php endif; ?>
                                &nbsp;&nbsp;
                                </td>
                                <td><a href="javascript:void(0);" title="激活" ><img src="/Public/Img/action_<?php echo ($vo["status"]); ?>.png" border="0"></a></td>
                                ...
                                <td>
                                    <?php if($vo["state"] == 2): ?>[ <a href="/index.php/conist/Popularizelaw/points?id=<?php echo ($vo["id"]); ?>">编辑分数 </a> ]<?php endif; ?>
                                    [ <a href="/index.php/conist/Popularizelaw/exam_list?id=<?php echo ($vo["id"]); ?>">试题列表 </a> ][ <a href="/index.php/conist/Popularizelaw/exam_type_add?id=<?php echo ($vo["id"]); ?>">编辑 </a> ] [ <a link="<?php echo U('Popularizelaw/exam_type_del/',array('id'=>$vo['id']));?>" href="javascript:void(0)" name="<?php echo ($vo["name"]); ?>" class="del">删除 </a> ]</td>
