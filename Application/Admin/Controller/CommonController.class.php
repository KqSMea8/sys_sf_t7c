...
/*    private function show_sub_menu() {
    ...
    private function show_sub_menu() {
        ...
        $sub_menu = array();
        if ($cache[$big]) {
            $admin_sub3_menu = C('admin_sub3_menu');
            $admin_sub4_menu = C('admin_sub4_menu');
            $cache = $cache[$big];
            foreach ($cache as $url => $title) {
                $url = $big == "Common" ? $url : "$big/$url";
                if($admin_sub3_menu[$url]){
                    $sub3 = [];
                    foreach ($admin_sub3_menu[$url] as $k => $v) {
                        // $sub3[] = array('url' => U("$k"), 'title' => $v);
                        //四级菜单
                        if($admin_sub4_menu[$k]){
                          $sub4 = [];
                          foreach ($admin_sub4_menu[$k] as $k4 => $v4) {
                            $sub4[] = array('url' => U("$k4"), 'title' => $v4);
                          }
                          $sub3[] = array('url' => U("$k"), 'title' => $v, 'sub4' => $sub4);
                        } else {
                           $sub3[] = array('url' => U("$k"), 'title' => $v, 'sub4' => 'false');
                        }
                        //四级菜单
                    }
                    $sub_menu[] = array('url' => U("$url"), 'title' => $title,'sub3'=>$sub3);
                }else{
                    $sub_menu[] = array('url' => U("$url"), 'title' => $title,'sub3'=>'false');
                }
            }
            return $sub_menu;
        } else {
